...
export const MonthlyStats: React.FC = () => {
  const [monthly, setMonthly] = useState<MonthlyPlayerStat[]>([]);
  ...
  useEffect(() => {
    const fetchMonthly = async () => {
      ...
      if (json.success && Array.isArray(json.monthly)) {
        setMonthly(json.monthly);
        if (json.monthly[0]?.month) setSelectedMonth(json.monthly[0].month);
      }
    };
  ...
  const months = useMemo(
    () => Array.from(new Set(monthly.map((row) => row.month))),
    [monthly]
  );

  const rows = monthly.filter((row) => row.month === selectedMonth);
      ...
      <div className="flex items-center justify-between mb-2">
        ...
        {months.length > 0 && (
          <select
            ...
            className="bg-gray-950 border border-gray-800 rounded-md px-2 py-1 text-[10px] text-gray-200"
          >
            {months.map((month) => (
          ...
          {rows.map((row) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-gray-500">{row.games}試合</span>
              </div>
